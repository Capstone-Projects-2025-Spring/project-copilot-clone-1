...
export function activate(context: vscode.ExtensionContext) {
	...
	const disposable = vscode.commands.registerCommand('educode.helloWorld', async () => {
		...
		if (editor) {
			...
			const res = await fetch('http://localhost:8000/logs', {method: 'POST', body: JSON.stringify({code:text, fileName: document.fileName, position: editor.selection.active}), headers: {'Content-Type': 'application/json'}});
			const body = await res.json() as {response: string}[];
			console.log(res.status, body.map(a=>a["response"]));
			suggestSnippet(editor,body[0].response, editor.selection.active);
